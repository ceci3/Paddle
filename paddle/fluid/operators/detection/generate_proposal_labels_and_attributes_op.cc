...
class GenerateProposalLabelsAndAttributesOp
 ...
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("GtAttrLbls"), true,
        platform::errors::NotFound("Input(GtAttrLbls) shouldn't be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("GtAttrLblWgts"), true,
        platform::errors::NotFound("Input(GtAttrLblWgts) shouldn't be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("GtAttrTgts"), true,
        platform::errors::NotFound("Input(GtAttrTgts) shouldn't be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("GtAttrTgtWgts"), true,
        platform::errors::NotFound("Input(GtAttrTgtWgts) shouldn't be null."));
    ...
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("AttributeLabels"), true,
        platform::errors::NotFound("Output(AttributeLabels) of "
                                   "GenerateProposalLabelsAndAttributesOp "
                                   "should not be null"));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("AttributeLabelWeights"), true,
        platform::errors::NotFound("Output(AttributeLabelWeights) of "
                                   "GenerateProposalLabelsAndAttributesOp "
                                   "should not be null"));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("AttributeTargets"), true,
        platform::errors::NotFound("Output(AttributeTargets) of "
                                   "GenerateProposalLabelsAndAttributesOp "
                                   "should not be null"));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("AttributeTargetWeights"), true,
        platform::errors::NotFound("Output(AttributeTargetWeights) of "
                                   "GenerateProposalLabelsAndAttributesOp "
                                   "should not be null"));
    ...
    int attr_lbl_nums = ctx->Attrs().Get<int>("attr_lbl_nums");
    int attr_tgt_nums = ctx->Attrs().Get<int>("attr_tgt_nums");
    ...
    ctx->SetOutputDim("AttributeLabels", {-1, attr_lbl_nums});
    ctx->SetOutputDim("AttributeLabelWeights", {-1, attr_lbl_nums});
    ctx->SetOutputDim("AttributeTargets", {-1, attr_tgt_nums});
    ctx->SetOutputDim("AttributeTargetWeights", {-1, attr_tgt_nums});
  }
...
template <typename T>
void GatherBoxesLabels(
    const platform::CPUDeviceContext& context, const Tensor& boxes,
    const Tensor& gt_boxes, const Tensor& gt_classes,
    const Tensor& gt_attr_lbls, const Tensor& gt_attr_lblwgts,
    const Tensor& gt_attr_tgts, const Tensor& gt_attr_tgtwgts,
    const std::vector<int>& fg_inds, const std::vector<int>& bg_inds,
    const std::vector<int>& gt_inds, const int attr_lbl_nums,
    const int attr_tgt_nums, Tensor* sampled_boxes, Tensor* sampled_labels,
    Tensor* sampled_gts, Tensor* sampled_attr_lbls,
    Tensor* sampled_attr_lblwgts, Tensor* sampled_attr_tgts,
    Tensor* sampled_attr_tgtwgts) {
  ...
  Tensor fg_attr_lbls, bg_attr_lbls, fg_attr_lblwgts, bg_attr_lblwgts,
      fg_attr_tgts, bg_attr_tgts, fg_attr_tgtwgts, bg_attr_tgtwgts;

  // attr lbl
  fg_attr_lbls.mutable_data<int>({fg_num, attr_lbl_nums}, context.GetPlace());
  CPUGather<int>(context, gt_attr_lbls, gt_label_inds_t, &fg_attr_lbls);
  bg_attr_lbls.mutable_data<int>({bg_num, attr_lbl_nums}, context.GetPlace());
  math::set_constant(context, &bg_attr_lbls, 0);
  Concat<int>(context, fg_attr_lbls, bg_attr_lbls, sampled_attr_lbls);
  // attr lblwgt
  fg_attr_lblwgts.mutable_data<T>({fg_num, attr_lbl_nums}, context.GetPlace());
  CPUGather<T>(context, gt_attr_lblwgts, gt_label_inds_t, &fg_attr_lblwgts);
  bg_attr_lblwgts.mutable_data<T>({bg_num, attr_lbl_nums}, context.GetPlace());
  math::set_constant(context, &bg_attr_lblwgts, 0);
  Concat<T>(context, fg_attr_lblwgts, bg_attr_lblwgts, sampled_attr_lblwgts);
  // attr tgt
  fg_attr_tgts.mutable_data<T>({fg_num, attr_tgt_nums}, context.GetPlace());
  CPUGather<T>(context, gt_attr_tgts, gt_label_inds_t, &fg_attr_tgts);
  bg_attr_tgts.mutable_data<T>({bg_num, attr_tgt_nums}, context.GetPlace());
  math::set_constant(context, &bg_attr_tgts, 0);
  Concat<T>(context, fg_attr_tgts, bg_attr_tgts, sampled_attr_tgts);
  // attr tgtwgt
  fg_attr_tgtwgts.mutable_data<T>({fg_num, attr_tgt_nums}, context.GetPlace());
  CPUGather<T>(context, gt_attr_tgtwgts, gt_label_inds_t, &fg_attr_tgtwgts);
  bg_attr_tgtwgts.mutable_data<T>({bg_num, attr_tgt_nums}, context.GetPlace());
  math::set_constant(context, &bg_attr_tgtwgts, 0);
  Concat<T>(context, fg_attr_tgtwgts, bg_attr_tgtwgts, sampled_attr_tgtwgts);
}
...
std::vector<Tensor> SampleRoisForOneImage(
    ...
    const int attr_lbl_nums, const int attr_tgt_nums, std::minstd_rand engine,
    bool use_random, bool is_cascade_rcnn, bool is_cls_agnostic) {
  // 1.1 map to original image
  auto im_scale = im_info.data<T>()[2];
  T im_height = im_info.data<T>()[0];
  T im_width = im_info.data<T>()[1];
  ...
  bool is_rpn_roi_valid = true;
  if (1 == rpn_rois.dims()[0]) {
    if (rpn_rois_dt[0] < 1e-6 and rpn_rois_dt[1] < 1e-6) {
      is_rpn_roi_valid = false;
    }
  }
  //  std::cout << "valid: " << is_rpn_roi_valid << std::endl;
  int proposals_num = gt_boxes.dims()[0] + 128;
  if (is_rpn_roi_valid) {
  ...
  if (is_rpn_roi_valid) {
    ...
    std::uniform_real_distribution<float> uniform(0, 1);
    const T* gt_boxes_dt = gt_boxes.data<T>();
    T* boxes_dt = boxes.data<T>();
    for (int i = 0; i < boxes.numel(); ++i) {
      if (i < gt_boxes.numel()) {
        boxes_dt[i] = gt_boxes_dt[i];
      } else {
        if (i % kBoxDim == 0) {
          T xc = uniform(engine);
          T yc = uniform(engine);
          T w = uniform(engine);
          T h = uniform(engine);
          T x0 = (xc - w * 0.5) * im_width;
          T y0 = (yc - h * 0.5) * im_height;
          T x1 = (xc + w * 0.5) * im_width;
          T y1 = (yc + h * 0.5) * im_height;
          x0 = std::max(x0, (T)0);
          y0 = std::max(y0, (T)0);
          x1 = std::min(x1, im_width - 1);
          y1 = std::min(y1, im_height - 1);
          if (x0 >= x1 || y0 >= y1) {
            x0 = (T)0;
            y0 = (T)0;
            x1 = im_width - 1;
  ...
  //  std::cout << "rpn_roi dim: " << rpn_rois.dims()[1] << std::endl;
  //  std::cout << proposals_num << " " << gt_boxes.dims()[0] << std::endl;
  //  const T* boxes_dt = boxes.data<T>();
  //  for (int i = 0; i < boxes.numel(); ++i) {
  //    if (i % kBoxDim == 0) std::cout << std::endl;
  //    std::cout << boxes_dt[i] << " ";
  //  }
  //  std::cout << std::endl;
  ...
  Tensor sampled_attr_lbls, sampled_attr_lblwgts;
  Tensor sampled_attr_tgts, sampled_attr_tgtwgts;
  int fg_num = fg_inds.size();
  int bg_num = bg_inds.size();
  ...
  sampled_attr_lbls.mutable_data<int>({boxes_num, attr_lbl_nums},
                                      context.GetPlace());
  sampled_attr_lblwgts.mutable_data<T>({boxes_num, attr_lbl_nums},
                                       context.GetPlace());
  sampled_attr_tgts.mutable_data<T>({boxes_num, attr_tgt_nums},
                                    context.GetPlace());
  sampled_attr_tgtwgts.mutable_data<T>({boxes_num, attr_tgt_nums},
                                       context.GetPlace());
  GatherBoxesLabels<T>(context, boxes, gt_boxes, gt_classes, gt_attr_lbls,
                       gt_attr_lblwgts, gt_attr_tgts, gt_attr_tgtwgts, fg_inds,
                       bg_inds, mapped_gt_inds, attr_lbl_nums, attr_tgt_nums,
                       &sampled_boxes, &sampled_labels, &sampled_gts,
                       &sampled_attr_lbls, &sampled_attr_lblwgts,
                       &sampled_attr_tgts, &sampled_attr_tgtwgts);
  ...
  res.emplace_back(sampled_attr_lbls);
  res.emplace_back(sampled_attr_lblwgts);
  res.emplace_back(sampled_attr_tgts);
  res.emplace_back(sampled_attr_tgtwgts);
  return res;
}

template <typename T>
class GenerateProposalLabelsAndAttributesKernel
 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    auto* gt_attr_lbls = context.Input<LoDTensor>("GtAttrLbls");
    auto* gt_attr_lblwgts = context.Input<LoDTensor>("GtAttrLblWgts");
    auto* gt_attr_tgts = context.Input<LoDTensor>("GtAttrTgts");
    auto* gt_attr_tgtwgts = context.Input<LoDTensor>("GtAttrTgtWgts");
    ...
    auto* attr_lbls = context.Output<LoDTensor>("AttributeLabels");
    auto* attr_lblwgts = context.Output<LoDTensor>("AttributeLabelWeights");
    auto* attr_tgts = context.Output<LoDTensor>("AttributeTargets");
    auto* attr_tgtwgts = context.Output<LoDTensor>("AttributeTargetWeights");
    ...
    int attr_lbl_nums = context.Attr<int>("attr_lbl_nums");
    int attr_tgt_nums = context.Attr<int>("attr_tgt_nums");
    ...
    bbox_outside_weights->mutable_data<T>(
        {n * batch_size_per_im, kBoxDim * class_nums}, context.GetPlace());
    attr_lbls->mutable_data<int>({n * batch_size_per_im, attr_lbl_nums},
                                 context.GetPlace());
    attr_lblwgts->mutable_data<T>({n * batch_size_per_im, attr_lbl_nums},
                                  context.GetPlace());
    attr_tgts->mutable_data<T>({n * batch_size_per_im, attr_tgt_nums},
                               context.GetPlace());
    attr_tgtwgts->mutable_data<T>({n * batch_size_per_im, attr_tgt_nums},
                                  context.GetPlace());
    ...
    auto gt_attr_lbls_lod = gt_attr_lbls->lod().back();
    auto gt_attr_lblwgts_lod = gt_attr_lblwgts->lod().back();
    auto gt_attr_tgts_lod = gt_attr_tgts->lod().back();
    auto gt_attr_tgtwgts_lod = gt_attr_tgtwgts->lod().back();
    for (int i = 0; i < n; ++i) {
      Tensor rpn_rois_slice;
      if (n == rpn_roi_batchsize) {
        if (rpn_rois_lod[i] == rpn_rois_lod[i + 1]) {
          lod0.emplace_back(num_rois);
          continue;
        }
        rpn_rois_slice = rpn_rois->Slice(rpn_rois_lod[i], rpn_rois_lod[i + 1]);
      } else {
        rpn_rois_slice = rpn_rois->Slice(rpn_rois_lod[0], rpn_rois_lod[1]);
        //        T* rpn_rois_slice_dt = rpn_rois_slice.data<T>();
        //        for (int i = 0; i < rpn_rois_slice.numel(); ++i) {
        //          std::cout << rpn_rois_slice_dt[i] << std::endl;
        //        }
        //        continue;
      }
      ...
      Tensor gt_attr_lbls_slice =
          gt_attr_lbls->Slice(gt_attr_lbls_lod[i], gt_attr_lbls_lod[i + 1]);
      Tensor gt_attr_lblwgts_slice = gt_attr_lblwgts->Slice(
          gt_attr_lblwgts_lod[i], gt_attr_lblwgts_lod[i + 1]);
      Tensor gt_attr_tgts_slice =
          gt_attr_tgts->Slice(gt_attr_tgts_lod[i], gt_attr_tgts_lod[i + 1]);
      Tensor gt_attr_tgtwgts_slice = gt_attr_tgtwgts->Slice(
          gt_attr_tgtwgts_lod[i], gt_attr_tgtwgts_lod[i + 1]);
      std::vector<Tensor> tensor_output = SampleRoisForOneImage<T>(
          dev_ctx, rpn_rois_slice, gt_classes_slice, is_crowd_slice,
          gt_boxes_slice, gt_attr_lbls_slice, gt_attr_lblwgts_slice,
          gt_attr_tgts_slice, gt_attr_tgtwgts_slice, im_info_slice,
          batch_size_per_im, fg_fraction, fg_thresh, bg_thresh_hi, bg_thresh_lo,
          bbox_reg_weights, class_nums, attr_lbl_nums, attr_tgt_nums, engine,
          use_random, is_cascade_rcnn, is_cls_agnostic);
      ...
      Tensor sampled_attr_lbls = tensor_output[5];
      Tensor sampled_attr_lblwgts = tensor_output[6];
      Tensor sampled_attr_tgts = tensor_output[7];
      Tensor sampled_attr_tgtwgts = tensor_output[8];
      ...
      AppendRois<int>(attr_lbls, num_rois * attr_lbl_nums, &sampled_attr_lbls);
      AppendRois<T>(attr_lblwgts, num_rois * attr_lbl_nums,
                    &sampled_attr_lblwgts);
      AppendRois<T>(attr_tgts, num_rois * attr_tgt_nums, &sampled_attr_tgts);
      AppendRois<T>(attr_tgtwgts, num_rois * attr_tgt_nums,
                    &sampled_attr_tgtwgts);
    ...
    attr_lbls->set_lod(lod);
    attr_lblwgts->set_lod(lod);
    attr_tgts->set_lod(lod);
    attr_tgtwgts->set_lod(lod);
    ...
    attr_lbls->Resize({num_rois, attr_lbl_nums});
    attr_lblwgts->Resize({num_rois, attr_lbl_nums});
    attr_tgts->Resize({num_rois, attr_tgt_nums});
    attr_tgtwgts->Resize({num_rois, attr_tgt_nums});
  }
};

class GenerateProposalLabelsAndAttributesOpMaker
 ...
 public:
  void Make() override {
    ...
    AddInput("GtAttrLbls",
             "(LoDTensor), This input is a 2D LoDTensor with shape [M, *].");
    AddInput("GtAttrLblWgts",
             "(LoDTensor), This input is a 2D LoDTensor with shape [M, *].");
    AddInput("GtAttrTgts",
             "(LoDTensor), This input is a 2D LoDTensor with shape [M, *].");
    AddInput("GtAttrTgtWgts",
             "(LoDTensor), This input is a 2D LoDTensor with shape [M, *].");
    ...
    AddOutput(
        ...
        "class_nums], "
        "each element indicates whether a box should contribute to loss.");
    AddOutput("AttributeLabels",
              "(LoDTensor), This output is a 2D LoDTensor with shape [P, *].");
    AddOutput("AttributeLabelWeights",
              "(LoDTensor), This output is a 2D LoDTensor with shape [P, *].");
    AddOutput("AttributeTargets",
              "(LoDTensor), This output is a 2D LoDTensor with shape [P, *].");
    AddOutput("AttributeTargetWeights",
              "(LoDTensor), This output is a 2D LoDTensor with shape [P, *].");
    ...
    AddAttr<int>("attr_lbl_nums", "attribute labels number.");
    AddAttr<int>("attr_tgt_nums", "attribute targets number.");
    AddAttr<bool>(
        "use_random",
...
REGISTER_OPERATOR(
    ...
    ops::GenerateProposalLabelsAndAttributesOp,
    ops::GenerateProposalLabelsAndAttributesOpMaker,
    paddle::framework::EmptyGradOpMaker<paddle::framework::OpDesc>,
    paddle::framework::EmptyGradOpMaker<paddle::imperative::OpBase>);
REGISTER_OP_CPU_KERNEL(generate_proposal_labels_and_attributes,
                       ops::GenerateProposalLabelsAndAttributesKernel<float>,
                       ops::GenerateProposalLabelsAndAttributesKernel<double>);
